  ...
  What we have:
  - /ask - Uses retrieve symbol (MCP Tier 2: precise lookup)
  - Existing commands: /deps, /find, /symbol, /test (mentioned in git status)
  ...
  /ask AuthHandler "How does token validation work?"
    → find_symbol (Tier 2)
  ...
  /ask suspicious_function "Why might this fail?"
    → find_symbol (Tier 2)
    → Targeted analysis

  The Key Insight

  Looking at the MCP Tool Tiers from CLAUDE.md:
  ...
  - find_symbol - Exact lookup ← This is what /ask uses
  ...
  2. /ask <symbol> "<question>" (TIER 2 - Understanding) ✅ Already created

  Uses: find_symbol  
  Purpose: Targeted Q&A about known symbol
  Example: /ask index_file "What does this do?"
